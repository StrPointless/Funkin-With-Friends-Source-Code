...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		blackScreen.frames = Paths.getSparrowAtlas('Menu');
		blackScreen.animation.addByPrefix('idle', 'BGBop', 24);
		...
		add(blackScreen);
		logoBl = new FlxSprite(300, -200);
		logoBl.frames = Paths.getSparrowAtlas('logoBumpin');
		logoBl.antialiasing = ClientPrefs.globalAntialiasing;
		logoBl.animation.addByPrefix('bump', 'logoBumpin2', 24);
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				FlxTween.tween(FlxG.camera, {y: FlxG.height}, 1.6, {ease: FlxEase.expoIn, startDelay: 0.5});
