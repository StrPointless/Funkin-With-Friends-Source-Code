...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'stage': //Week 1
				var bg:BGSprite = new BGSprite('stageback', 0, -175, 1, 1);
				// + equals image down - equals image up
				bg.screenCenter(X);
				bg.scale.set(2.7,2.7);
				add(bg);
				bgppl = new BGSprite('bgppls', -100, 130, 1, 1, ['BGppls']);
				bgppl.updateHitbox();
				bgppl.scale.set(1.9,1.9);
				add(bgppl);
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				switch (swagCounter)
				{
					...
					case 3:
						...
						dad.animation.play('go');
						boyfriend.animation.play('go');
					case 4:
				}
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
						...
						switch (Math.abs(daNote.noteData))
						{
							case 0:
								animToPlay = 'singLEFT';
							if(SONG.player2=='ffv' && curSong != 'Dont' && curSong != 'Big Vibin')
							{
								health -= 0.02;
							}
							case 1:
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'STR Cool camera zoom thing':
				var val1:Float = Std.parseFloat(value1);
				FlxTween.tween(camera, {'zoom': val1});
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
				...
				'ur_good', 'hype', 'two_keys', 'toastie', 'debugger','week1_win']);
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					if(storyWeek == 1)
					{
						var achieve:String = checkForAchievement(['week1_win']);
	...
	override function stepHit()
	{
		...
		// That first ah , ah, ah part

		if(curSong == 'Ultra Battle' && curStep > 516)
		{
			FlxTween.tween(camera, {'zoom': 0.45}, 1, {ease: FlxEase.expoInOut});
		}
		if(curSong == 'Big Vibin' && curStep > 523)
		{
			FlxTween.tween(camera, {'zoom': 0.65}, 0.5, {ease: FlxEase.expoInOut});
		}
